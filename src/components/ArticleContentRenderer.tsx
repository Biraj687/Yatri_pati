      ...
      if (trimmedLine.endsWith(':') || trimmedLine.length < 60) {
        ...
        if (isMainHeading) {
          elements.push(
            <h2
              key={`heading-${index}`}
              className={`font-bold text-secondary-700 dark:text-secondary-300 mb-4 mt-6 first:mt-0 text-2xl md:text-3xl ${headingClassName}`}
            >
              {headingText}
            </h2>
          );
          ...
          elements.push(
            <h3
              key={`heading-${index}`}
              className={`font-bold text-secondary-700 dark:text-secondary-300 mb-4 mt-6 first:mt-0 text-xl md:text-2xl ${headingClassName}`}
            >
              {headingText}
            </h3>
          );
      ...
      if (trimmedLine.length < 100 && !trimmedLine.match(/[।.!?]$/)) {
        elements.push(
          <h4 
            ...
            className={`font-semibold text-secondary-600 dark:text-secondary-300 mb-3 mt-4 text-lg ${headingClassName}`}
          >
            {trimmedLine}
          </h4>
        );
