...
import logo1Svg from '@assets/logo_1.svg'
...
export function Navbar() {
  ...
  return (
    <nav className="bg-white dark:bg-gray-900 sticky top-0 z-50 transition-colors duration-300 flex flex-col items-center w-full border-b-2 border-brand-red dark:border-brand-red">
      {/* Very Top Row: Date | Socials (Ticker removed) */}
        ...
        <div className="flex-1 flex items-center justify-end gap-3 md:gap-4">
           {config.socialLinks.map((link: typeof config.socialLinks[number]) => (
              <a 
                ...
                className="text-gray-800 dark:text-gray-300 hover:text-secondary-500 dark:hover:text-secondary-400 transition-colors" 
      ...
      <div className="w-full relative flex items-center justify-between md:justify-center px-4 md:px-8 lg:px-[5rem] py-4 border-b border-gray-50 dark:border-gray-800 md:border-b-0">
        ...
        <Link to="/" className="flex items-center">
          <img src={isDark ? logo1Svg : logoSvg} alt="Nepal Explain Logo" className="h-12 md:h-16 lg:h-20 w-auto" />
        </Link>
        ...
        <div className="flex items-center gap-4 md:absolute md:right-8 lg:right-[5rem]">
          <button
            onClick={toggleTheme}
            className="text-gray-800 dark:text-gray-200 hover:text-brand-red dark:hover:text-brand-red transition-colors"
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 lg:gap-8 max-w-7xl">
          {config.navigation.map((item: typeof config.navigation[number]) => (
            <div key={item.label} className="relative group flex items-center h-12">
              <Link 
                to={item.path} 
                className={`font-bold px-2 text-base lg:text-lg transition-colors duration-300 flex items-center h-full ${isActive(item.path) ? 'text-brand-red' : 'text-gray-900 dark:text-gray-100 hover:text-brand-red'}`}
              >
                {item.label}
                ...
                <div className="absolute top-full left-1/2 -translate-x-1/2 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-b shadow-xl min-w-[160px] z-50 hidden group-hover:block py-2">
                  {item.dropdownItems.map((sub: typeof item.dropdownItems[number]) => (
                    <Link 
                      key={sub.label} 
                      to={sub.path} 
                      className="block px-6 py-2.5 text-sm text-gray-800 dark:text-gray-200 hover:text-brand-red dark:hover:text-brand-red hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                    >
                      {sub.label}
                    </Link>
                  ))}
        ...
        <div className="p-6 h-full flex flex-col">
          <div className="flex justify-between items-center mb-8">
            <div className="flex items-center gap-2">
              <img src={isDark ? logo1Svg : logoSvg} alt="Nepal Explain Logo" className="h-12 w-auto" />
            </div>
                ...
                return (
                  <div key={item.label}>
                    <div className="flex items-center justify-between">
                      <Link
                        ...
                        className={`block py-3 px-2 text-lg font-bold transition-colors flex-1 ${
                          isActive(item.path) ? 'text-brand-red' : 'text-gray-800 dark:text-gray-200 hover:text-brand-red'
                        }`}
                        onClick={() => setMobileMenuOpen(false)}
                      >
                        {item.label}
                    ...
                    {item.hasDropdown && item.dropdownItems && isDropdownOpen && (
                      <div className="pl-4 space-y-2 mb-4 border-l-2 border-secondary-200 dark:border-secondary-800 ml-2">
                        {item.dropdownItems.map((sub: typeof item.dropdownItems[number]) => (
                          <Link
                            key={sub.label}
                            to={sub.path}
                            className="block py-2 text-gray-600 hover:text-secondary-500 dark:hover:text-secondary-400 text-sm"
                            onClick={() => {
