  ...
  useEffect(() => {
    const loadArticles = async () => {
      try {
        ...
        const placed = allArticles.filter((a) => (a as any).homepageSectionId === 'destination');
        setArticles((placed.length > 0 ? placed : allArticles).slice(0, 6));
      } catch (error) {
  ...
  if (loading) {
    return (
      <section className="w-full py-16 bg-black">
          ...
          <h2 className="text-white text-4xl font-bold text-left mb-12 font-akshar">{title}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {articles.map((article) => (
            <Link key={article.id} to={`/news/${article.id}`} className="flex flex-col group transition-all duration-300">
              ...
              {/* Title */}
              <h3 style={{ lineHeight: '2.5rem' }} className="text-white text-2xl md:text-3xl font-bold mb-4 flex-grow line-clamp-2 overflow-hidden">
                {article.title}
              </h3>
