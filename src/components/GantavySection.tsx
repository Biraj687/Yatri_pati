        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {articles.map((article) => (
            <Link key={article.id} to={`/news/${article.id}`} className="flex flex-col group transition-all duration-300">
              ...
              {/* Title */}
              <h3 className="text-white text-2xl md:text-3xl font-bold leading-[2.45rem] mb-4 flex-grow group-hover:text-primary-400 transition-colors line-clamp-2 overflow-hidden">
                {article.title}
              </h3>
