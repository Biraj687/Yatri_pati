    ...
    return (
      ...
      >
        ...
        <div className="space-y-2">
          <h3 className={`${titleClasses} text-gray-900 dark:text-gray-100 group-hover:text-brand-red transition-colors line-clamp-${truncateLines}`}>
            {article.title}
          </h3>
              ...
              {rightArticles.map(article => (
                <div key={article.id} className="flex gap-4 items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-gray-900 dark:text-gray-100 group-hover:text-brand-red transition-colors text-2xl md:text-3xl font-bold leading-tight mb-3 drop-shadow-md">
                      {article.title}
                    </h4>
                    {showAuthor && article.author && (
  ...
  return (
    <section className={`py-12 w-full transition-colors duration-300 ${className}`}>
      <div className={`max-w-7xl mx-auto px-4 md:px-8 lg:px-[5rem] ${containerClassName}`}>
        <h2 className={`text-5xl font-bold text-secondary-700 dark:text-secondary-300 mb-8 border-b-4 border-brand-red pb-3 text-left ${titleClassName}`}>
          {title}
        </h2>
