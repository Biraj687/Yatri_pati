...
export function CompactArticle({
  ...
  return (
    <article
      ref={ref}
      className={`relative group rounded-xl border border-gray-200 dark:border-gray-700 transition-all duration-300 opacity-0 translate-y-4 text-left bg-white dark:bg-gray-900 h-full flex flex-col
        ${minimal ? 'p-2' : 'p-4'} 
        ${isVisible ? 'opacity-100 translate-y-0' : ''}`}
      style={{ animationDelay: `${index * 0.1}s`, transition: 'opacity 0.6s ease, transform 0.6s ease' }}
    >
      <Link to={articleUrl} className={`flex flex-row gap-4 items-center ${minimal ? 'h-auto' : 'h-full'}`}>
        {/* Thumbnail */}
        <div className={`overflow-hidden rounded-lg flex-shrink-0
          ${minimal ? 'w-24 h-20 md:w-28 md:h-24' : 'w-48 h-36 md:w-56 md:h-40'}`}>
          <OptimizedImage
            src={displayImage}
            alt={article.title}
        ...
        <div className="flex-1 flex flex-col justify-center min-w-0">
          <h3 className={`font-bold text-[#5e5ca7] dark:text-[#5e5ca7] group-hover:text-brand-red transition-colors mb-1.5 leading-[1.25rem] line-clamp-2
