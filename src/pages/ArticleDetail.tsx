...
export function ArticleDetail() {
  ...
  // Scroll to top when article ID chanea0031ges
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [id]);

  const { article, isLoading: articleLoading } = useArticle({
    id: id || "",
    enabled: !!id,
  });
  ...
  if (!article) {
    return (
      <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-[5rem] py-12 min-h-[60vh] flex flex-col items-center justify-center">
        ...
        <Link
          to="/"
          className="text-brand-red hover:underline flex items-center gap-2"
        >
  ...
  const displayedRelatedArticles = (
    relatedError ? fallbackRelatedArticles : relatedArticles
  ).filter((a): a is Article => a !== null);
  // Clean date by removing trailing " 0" if present
  const cleanedDate = article.date
    ? article.date.replace(/ 0$/, "")
    : article.date;
  const siteName = config?.siteName || "Nepal Explain";
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-[5rem] py-14 flex flex-col lg:flex-row gap-16 text-left">
          ...
          <div className="flex-1">
            ...
            <header className="mb-10">
              <h1 className="text-[1.75rem] md:text-[2.25rem] lg:text-[2.625rem] font-bold leading-[1.3] mb-6 text-gray-900 dark:text-white border-l-8 border-brand-red pl-6">
                {article.title}
              </h1>
            ...
            <div className="mt-12 pt-8 border-t border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-4">
                <button
                  onClick={handleShare}
                  className="inline-flex items-center gap-2 px-5 py-3 bg-brand-red hover:bg-brand-red/90 text-white rounded-lg transition-colors shadow-lg shadow-brand-red/20"
                >
                  <FiShare2 />
                  सेयर गर्नुहोस्
                </button>
          ...
          <aside className="w-full lg:w-96 flex-shrink-0">
            <div className="sticky top-24">
              <h3 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white border-b-4 border-brand-red pb-2 inline-block">
                सम्बन्धित समाचार
              </h3>
