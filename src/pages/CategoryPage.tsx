  ...
  if (error) {
    return (
      <main className="w-full py-12 min-h-[60vh] bg-white dark:bg-gray-900">
        <section className="max-w-7xl mx-auto px-4 md:px-8 lg:px-[5rem]">
          <Helmet>
            <title>{`${displayName} - ${config?.siteName || 'Nepal Explain'}`}</title>
            <meta name="description" content={`Read the latest ${displayName} news and updates on ${config?.siteName || 'Nepal Explain'}.`} />
          </Helmet>

          <div className="border-b border-secondary-500 mb-10 pb-4">
            <h1 className="text-3xl md:text-4xl font-bold text-secondary-700 dark:text-secondary-300 uppercase">
              {displayName}
            </h1>
          </div>
