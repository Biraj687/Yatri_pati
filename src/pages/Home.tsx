...
export function Home() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const latestPlaced = combined.filter((a) => (a as any).homepageSectionId === 'latest');

        if (latestPlaced.length > 0) {
          const placedFeatured = latestPlaced.find((a) => (a as any).homepageSlot === 'featured') || null;
          const placedList = latestPlaced.filter((a) => (a as any).homepageSlot !== 'featured');
          setFeatured(placedFeatured);
          setArticles(placedList);
        } else {
  ...
  if (loading || configLoading) {
    return (
      <>
        <Helmet>
          <title>{config?.siteName || 'Nepal Explain'} - Loading News...</title>
        </Helmet>
  ...
  return (
    <>
      <Helmet>
        <title>{config?.siteName || 'Nepal Explain'} - Latest News from Nepal</title>
      ...
      <main className="w-full py-12 dark:bg-gray-900 transition-colors duration-300">
        <section className="max-w-7xl mx-auto px-4 md:px-8 lg:px-[5rem]">
          <h2 className="text-4xl font-bold text-secondary-700 dark:text-secondary-300 mb-10 text-left uppercase border-b-4 border-brand-red inline-block pb-2 font-akshar">
