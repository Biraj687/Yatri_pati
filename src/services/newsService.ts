...
function getStoredDashboardArticles(): any[] {
  if (typeof window === 'undefined') return [];
  const parsed = safeParseJson(window.localStorage?.getItem('nepal_explain_dashboard_articles') || null);
  return Array.isArray(parsed) ? parsed : [];
}

function toArticleFromDashboard(dashboardArticle: any): Article | null {
  if (!dashboardArticle) return null;
  const title = String(dashboardArticle.title || '').trim();
  if (!title) return null;

  const createdAt = dashboardArticle.createdAt || dashboardArticle.updatedAt || new Date().toISOString();
  const authorName =
    (Array.isArray(dashboardArticle.authors) && dashboardArticle.authors[0]?.name) ||
    dashboardArticle.author ||
    'बिराज प्याकुरेल';

  const article: Article = {
    id: dashboardArticle.id || dashboardArticle._id || `dash-${Math.random()}`,
    title,
    image: dashboardArticle.featured_image || dashboardArticle.image || dashboardArticle.imageUrl || '',
    excerpt: dashboardArticle.excerpt || dashboardArticle.summary || dashboardArticle.description || '',
    author: String(authorName),
    date: dashboardArticle.date || new Date(createdAt).toLocaleDateString('ne-NP'),
    category: dashboardArticle.category || undefined,
    authorAvatar: dashboardArticle.authorAvatar || ppImage,
    content: dashboardArticle.content || undefined,
    tags: dashboardArticle.tags || undefined,
    slug: dashboardArticle.slug || undefined,
    videoUrl: dashboardArticle.video_url || dashboardArticle.videoUrl || undefined,
    thumbnailImage: dashboardArticle.thumbnailImage || dashboardArticle.thumbnail_url || dashboardArticle.thumbnailUrl || undefined,
    views: dashboardArticle.views || 0,
    // Placement metadata (used by homepage sections)
    homepageSectionId: dashboardArticle.homepageSectionId || undefined,
    homepageSlot: dashboardArticle.homepageSlot || undefined,
    status: dashboardArticle.status || undefined,
  };

  return article;
}

function getStoredSiteSettings(): any | null {
  if (typeof window === 'undefined') return null;
  return safeParseJson(window.localStorage?.getItem('nepal_explain_settings') || null);
}

function settingsToSiteConfigOverrides(settings: any): Partial<SiteConfig> {
  if (!settings || typeof settings !== 'object') return {};

  const overrides: Partial<SiteConfig> = {};
  if (typeof settings.siteName === 'string') overrides.siteName = settings.siteName;

  if (settings.sectionTitles && typeof settings.sectionTitles === 'object') {
    const st = settings.sectionTitles;
    const sectionTitles: Record<string, string> = {};
    const putIfString = (key: string, value: unknown) => {
      if (typeof value === 'string' && value.trim()) sectionTitles[key] = value;
    };

    putIfString('latest', st.latest);
    putIfString('trending', st.trending);
    putIfString('packages', st.packages);
    putIfString('hospitality', st.hospitality);
    putIfString('hotels', st.hotels);
    putIfString('destination', st.destination || st.destinations);

    if (Object.keys(sectionTitles).length > 0) overrides.sectionTitles = sectionTitles;
  }

  if (Array.isArray(settings.navigation)) {
    overrides.navigation = settings.navigation;
  }

  if (settings.socialLinks && typeof settings.socialLinks === 'object' && !Array.isArray(settings.socialLinks)) {
    const socials = settings.socialLinks;
    const mapped = Object.entries(socials).flatMap(([platform, url]) => {
      if (typeof url !== 'string') return [];
      const trimmed = url.trim();
      if (!trimmed) return [];
      return [{ platform, url: trimmed }];
    });
    if (mapped.length) overrides.socialLinks = mapped;
  }

  return overrides;
}

function mergeSiteConfig(base: SiteConfig, overrides: Partial<SiteConfig>): SiteConfig {
  return {
    ...base,
    ...overrides,
    logo: {
...
// Build a NewsDataResponse from local mock data + any dashboard articles stored
// in localStorage. Used both when VITE_USE_MOCK_DATA=true and as a graceful
// fallback when the real API is unreachable (e.g. on Vercel before a backend
// is configured), so the homepage never crashes into an ErrorState.
function buildMockNewsData(storedDashboardArticles: Article[]): NewsDataResponse {
  const mockArticles = mockTargetArticles
    .map(normalizeArticle)
    .filter((article): article is Article => article !== null);
  return {
    hero: normalizeArticle(mockHeroArticle)!,
    featured: normalizeArticle(mockFeaturedArticle)!,
    articles: [...storedDashboardArticles, ...mockArticles]
  };
}

export const fetchNewsData = async (): Promise<NewsDataResponse> => {
  const storedDashboardArticles = getStoredDashboardArticles()
    .filter((a) => a && (a.status === 'published' || a.status === 'draft' || !a.status))
    .map(toArticleFromDashboard)
    .filter((a): a is Article => a !== null);

  if (USE_MOCK) {
    return new Promise<NewsDataResponse>((resolve) => {
      setTimeout(() => resolve(buildMockNewsData(storedDashboardArticles)), 800);
    });
  }
  ...
  try {
    ...
    return {
      ...
      articles: rawArticles
        ...
        .map(sanitizeArticle)
        .filter((article): article is Article => article !== null)
        .concat(storedDashboardArticles)
    };
  } catch (error) {
    console.error("API Fetch Error:", error);
    // Graceful fallback: if the backend is unreachable (common on Vercel when
    // no real API is configured yet), return mock data instead of throwing so
    // the homepage renders content rather than an ErrorState.
    return buildMockNewsData(storedDashboardArticles);
  }
};
...
const mockSiteConfig: SiteConfig = {
  ...
  footer: {
    aboutText: 'प्रत्येक मानिसले अर्को व्यक्तिलाई प्राकृतिक रूपमा आफू सरह सम्झनुपर्दछ । थोमस हब्स। Nepal Explain नेपालको एक अग्रणी न्युज पोर्टल हो जसले सत्य, तथ्य र निष्पक्ष समाचार सम्प्रेषण गर्दछ।',
    copyright: `© ${new Date().getFullYear()} Nepal Explain`,
    columns: [
      {
        title: 'स्थान',
        links: [
...
export const fetchSiteConfig = async (): Promise<SiteConfig> => {
  ...
  const storedOverrides = settingsToSiteConfigOverrides(storedSettings);
  if (USE_MOCK) {
    return new Promise<SiteConfig>((resolve) => {
      setTimeout(() => resolve(mergeSiteConfig(mockSiteConfig, storedOverrides)), 500);
    });
  }
  ...
  try {
    ...
    return mergeSiteConfig(response.data as SiteConfig, storedOverrides);
  } catch (err) {
    console.warn("Config fetch failed, using mock fallback", err);
    return mergeSiteConfig(mockSiteConfig, storedOverrides);
  }
};
