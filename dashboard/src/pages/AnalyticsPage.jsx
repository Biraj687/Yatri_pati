...
export function AnalyticsPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-secondary-700">Analytics</h1>
      ...
      {analytics && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-lg font-semibold text-secondary-700 mb-4">Article Views (7 days)</h2>
            <BarChart data={analytics.articleViews} dates={analytics.dates} />
          </div>

          {/* Ad Performance Chart */}
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-lg font-semibold text-secondary-700 mb-4">Ad Impressions (7 days)</h2>
            <BarChart data={analytics.adImpressions} dates={analytics.dates} color="orange" />
          </div>

          {/* CTR Analysis */}
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-lg font-semibold text-secondary-700 mb-4">Article Click Rate</h2>
            <LineChart data={analytics.articleClicks} />
          </div>

          {/* Ad Click Analysis */}
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-lg font-semibold text-secondary-700 mb-4">Ad Click Performance</h2>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="p-6 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-secondary-700">Recent Articles</h2>
        </div>
...
function MetricCard({ title, value, change, color }) {
  ...
  return (
    <div className={`${colors[color] || colors.blue} rounded-lg p-6`}>
      <p className="text-sm font-medium opacity-75">{title}</p>
      <p className="text-3xl font-bold mt-2">{Number(value).toLocaleString()}</p>
