...
export const uploadFileSchema = z.object({
    file: z.instanceof(File, { message: 'Must be a file' })
        ...
        .refine(file => ['image/jpeg', 'image/png', 'image/gif', 'application/pdf', 'text/csv'].includes(file.type), 'Invalid file type'),
...
// ============ Helper Functions ============
export async function validateNewsArticle(data) {
    return createNewsSchema.parseAsync(data);
}
export async function validateAdvertisement(data) {
    return createAdvertisementSchema.parseAsync(data);
}
export async function validateFileUpload(data) {
    return uploadFileSchema.parseAsync(data);
}
export async function validateSettings(data) {
