...
export function DashboardProvider({ children }) {
  const [articles, setArticles] = useState(() => {
    try {
      const stored = localStorage.getItem('nepal_explain_dashboard_articles');
  ...
  const [categories, setCategories] = useState(() => {
    try {
      const stored = localStorage.getItem('nepal_explain_dashboard_categories');
      const parsed = stored ? JSON.parse(stored) : null;
  ...
  useEffect(() => {
    try {
      localStorage.setItem('nepal_explain_dashboard_articles', JSON.stringify(articles));
    } catch {
      // ignore
    }
  }, [articles]);
  ...
  useEffect(() => {
    try {
      localStorage.setItem('nepal_explain_dashboard_categories', JSON.stringify(categories));
    } catch {
      // ignore
    }
  }, [categories]);
