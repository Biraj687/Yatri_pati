...
import { AsyncState, createAsyncState, apiResponseToStateUpdate, withAsyncState } from '../types/async';
...
export function useAsync<T = unknown>(initialData?: T) {
  const [state, setState] = useState<AsyncState<T>>(() => 
    createAsyncState<T>(initialData ?? null)
  );
  ...
  const execute = useCallback(async (
    asyncFn: () => Promise<T | ApiResponse<T>>,
    options?: {
   ...
   */
  const executeWithAsyncState = useCallback(async (
   ...
   */
  const updateFromApiResponse = useCallback((response: ApiResponse<T>) => {
  ...
  const reset = useCallback((newData?: T) => {
    setState(createAsyncState<T>(newData ?? initialData ?? null));
    if (abortControllerRef.current) {
      abortControllerRef.current.abort();
      abortControllerRef.current = null;
    }
 ...
 */
export function usePaginatedAsync<T = unknown>(initialPage: number = 1, initialLimit: number = 20) {
  ...
  const loadMore = useCallback(async (
    loadFn: (pageNum: number, limitNum: number) => Promise<{ data: T[]; total: number; hasMore: boolean }>,
    options?: {
      append?: boolean;
