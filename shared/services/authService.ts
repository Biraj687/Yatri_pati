...
class AuthService {
  ...
  private tokenKey = 'nepal_explain_auth_token';
  private refreshTokenKey = 'nepal_explain_refresh_token';
  private userKey = 'nepal_explain_user';
  ...
  hasRole(role: User['role'] | User['role'][]): boolean {
    ...
    if (Array.isArray(role)) {
      return role.length === 0 || role.includes(user.role);
    }
  ...
  private async mockLogin(credentials: LoginCredentials): Promise<AuthResponse> {
    ...
    if (credentials.email === 'admin@nepalexplain.com' && credentials.password === 'password') {
      const response: AuthResponse = {
        user: {
          id: '1',
          email: 'admin@nepalexplain.com',
